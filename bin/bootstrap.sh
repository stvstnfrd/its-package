#!/bin/sh
set -aeu
GIT_REPO=its-package
GIT_USER=stvstnfrd
GIT_BRANCH=master
# wget -O /tmp/bootstrap.sh https://raw.githubusercontent.com/${GIT_USER}/${GIT_REPO}/${GIT_BRANCH}/dist/${ID}/${VERSION_CODENAME}/Bootstrap.sh && less /tmp/bootstrap.sh && sh /tmp/bootstrap.sh
command_exists() {
...
;
echo "deb [signed-by=/usr/local/share/keyrings/${GIT_REPO}.gpg] https://raw.githubusercontent.com/${GIT_USER}/${GIT_REPO}/${GIT_BRANCH}/dist/${ID} ${VERSION_CODENAME} main" \
| sudo tee "/etc/apt/sources.list.d/${GIT_REPO}.list"
...
echo "${PACKAGE_KEY}" \
| base64 --decode \
| sudo tee "${PATH_KEYRINGS}/${GIT_REPO}.gpg" >/dev/null \
;
sudo apt-get update
# vim: nowrap tw=0
