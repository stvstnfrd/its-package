#!/bin/sh
set -aeu
# wget -O /tmp/bootstrap.sh https://raw.githubusercontent.com/stvstnfrd/its-package/master/dist/${ID}/${VERSION_CODENAME}/Bootstrap.sh && less /tmp/bootstrap.sh && sh /tmp/bootstrap.sh
command_exists() {
...
| base64 --decode \
| sudo tee "${PATH_KEYRINGS}/docker.gpg" >/dev/null \
;
sudo apt-get update
sudo DEBIAN_FRONTEND=noninteractive apt-get install --yes "${PACKAGE_NAME}"
sudo DEBIAN_FRONTEND=noninteractive apt-get install --yes --no-install-recommends docker-ce docker-ce-cli docker-compose-plugin docker-scan-plugin
